...
::   %5 Name of package to install
::     (ModelMetrics, caret, dplyr, etc..)
...
IF [%ARG_PARSE%]==[] (
	...
	SET PKG_TO_INSTALL=%ARG_PARSE%
)


REM Install packages locally
%RSCRIPT_PATH% %SCRIPTS_INSTALL_PATH% %PKG_SOURCE_PATH% %PKG_INSTALL_PATH% %PKG_TO_INSTALL%
IF %ERRORLEVEL% NEQ 0 (
	EXIT %ERRORLEVEL%
)

REM Verify package installs
%RSCRIPT_PATH% %SCRIPTS_VERIFY_PATH% %PKG_SOURCE_PATH% %PKG_INSTALL_PATH% %PKG_TO_INSTALL%
IF %ERRORLEVEL% NEQ 0 (
	EXIT %ERRORLEVEL%
)
